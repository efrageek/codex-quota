...
export function buildClaudeUsageLines(payload, flags = {}) {
	...
	if (usage && typeof usage === "object") {
		...
		if (fable && fable.remaining !== null) {
			...
			lines.push(`Fable weekly: ${printBar(fable.remaining)} ${Math.round(fable.remaining)}% left ${reset}`.trimEnd());
			renderedLimitKeys.add("model:fable:weekly");
			renderedUsage = true;
		}
		for (const limit of getClaudeLimitWindows(usage)) {
			...
			const labelPrefix = `${limit.label}:`.padEnd(14);
			lines.push(`${labelPrefix} ${printBar(limit.parsed.remaining)} ${Math.round(limit.parsed.remaining)}% left ${reset}`.trimEnd());
			renderedUsage = true;
		}
	}
...
export function printHelp() {
	console.log(`${PRIMARY_CMD} - Manage and monitor OpenAI Codex, Claude, Factory, and Grok accounts
Version: ${getPackageVersion()}

Usage:
  ${PRIMARY_CMD} <namespace> [command] [options]
  ${PRIMARY_CMD} [label]                       Check quota for all accounts (Codex + Claude + Factory + Grok)
...
Examples:
  ${PRIMARY_CMD}                   Check quota for all accounts (Codex + Claude + Factory + Grok)
...
Description:
  ...
  product split, prepaid balance, and period reset). Tokens are loaded from:
    ...
    2. pi / shuvpi / shuvhelm auth.json (xai-oauth)
...
export function buildFactoryUsageLines(account, payload, flags = {}) {
	...
	if (limit > 0) {
		const remaining = Math.max(0, Math.min(100, 100 - (usage.percent ?? 0)));
		lines.push(`Monthly:  ${printBar(remaining)} ${Math.round(remaining)}% left (${formatTokenCount(used)} / ${formatTokenCount(limit)} tokens)`);
	} else {
		lines.push(`Monthly:  ${formatTokenCount(used)} tokens used — no limit set`);
	}
...
export function buildGrokUsageLines(account, payload, flags = {}) {
	...
	const labelDisplay = account?.label ? ` (${account.label})` : "";
	const emailDisplay = formatEmailDisplay(account?.email, flags);
	const tierDisplay = account?.tier != null ? ` · tier ${account.tier}` : "";
	const header = `Grok${labelDisplay}${emailDisplay}${tierDisplay}`;
	// Match Claude label column width ("Weekly limit: ")
	const labelWidth = 13;
	...
	if (usedPercent != null && Number.isFinite(usedPercent)) {
		...
		lines.push(`${"Credits:".padEnd(labelWidth)} ${printBar(remaining)} ${Math.round(remaining)}% left ${reset}`.trimEnd());
	} else {
		lines.push(`${"Credits:".padEnd(labelWidth)} n/a`);
	}
	...
	for (const product of products) {
		...
		const name = `${product.product}:`.padEnd(labelWidth);
		lines.push(`${name} ${printBar(productRemaining)} ${Math.round(productRemaining)}% left`);
	}
