 ...
 * By default, shows Codex, Claude, and Factory accounts
 * @param {string[]} args - Non-flag arguments (e.g., label filter)
 * @param {{ json: boolean, local?: boolean }} flags - Parsed flags
 * @param {"all" | "codex" | "claude" | "factory"} scope - Which accounts to show
 */
export async function handleQuota(args, flags, scope = "all") {
	...
	const showFactory = scope === "all" || scope === "factory";
	...
	const useProxxForCodex = showCodex && Boolean(proxxToken);
	...
	if (!hasCodexResults && !hasClaudeResults && !hasFactoryResults) {
		if (flags.json) {
			console.log(JSON.stringify({ 
				success: false, 
	...
	if (proxxResults) {
		for (const account of proxxResults.accounts) {
			const lines = buildProxxAccountUsageLines(account);
			const boxLines = drawBox(lines);
			console.log(boxLines.join("\n"));
		}
	}
...
export async function handleProxxQuota(args, flags) {
	...
	// Human-readable display using box format
	for (const account of accounts) {
		const lines = buildProxxAccountUsageLines(account);
		const boxLines = drawBox(lines);
		console.log(boxLines.join("\n"));
