...
function normalizeCodexAccount(entry, index, now) {
	...
	const account = {
		...
		plan: sanitizeDisplayText(entry.planType ?? entry.plan_type ?? usage.plan_type ?? usage.planType),
		status,
		windows,
	};
	...
	return {
		account: {
			...
			plan: sanitizeDisplayText(
				entry.subscriptionType ?? entry.rateLimitTier
					?? accountObject.plan ?? accountObject.plan_type ?? accountObject.planType,
			),
			status,
			windows,
		},
...
function normalizeGrokAccount(entry, index, now) {
	...
	const status = entryError
		...
		: windows.length === 0
			? "unavailable"
			: windows.some(window => window.remainingPercent <= 20) ? "attention" : "ok";
	const tier = entry.tier === null || entry.tier === undefined ? null : `Tier ${entry.tier}`;
	return {
		account: {
			label: sanitizeDisplayText(entry.label) ?? `Account ${index + 1}`,
			email: maskEmail(entry.email),
			plan: sanitizeDisplayText(entry.plan ?? entry.planType ?? tier),
			status,
			windows,
		},
